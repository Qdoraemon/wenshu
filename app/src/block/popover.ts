...
export const initBlockPopover = (app: App) => {
  ...
  document.addEventListener(
    "mouseover",
    (event: MouseEvent & { target: HTMLElement; path: HTMLElement[] }) => {
      ...
      if (aElement) {
        ...
        if (aElement.classList.contains("av__cell")) {
          if (aElement.classList.contains("av__cell--header")) {
            ...
            if (
              textElement.scrollWidth > textElement.clientWidth + 0.5 ||
              desc
            ) {
              if (desc) {
                tip = `${getCellText(
                  aElement
                )}<div class='ft__on-surface'>${escapeAriaLabel(desc)}</div>`;
              } else {
            ...
            if (
              aElement.firstElementChild?.getAttribute("data-type") === "url"
            ) {
              if (aElement.firstElementChild.textContent.indexOf("...") > -1) {
                tip = Lute.EscapeHTMLStr(
          ...
          aElement.parentElement.parentElement.classList.contains(
            "av__views"
          ) &&
          ...
          tip = tip
            ? `<span style="word-break: break-all">${tip.substring(
                0,
                Constants.SIZE_TITLE
              )}</span>${
                title
                  ? '<div class="fn__hr"></div><span>' + title + "</span>"
                  : ""
              }`
            : title;
          tooltipClass = "href";
        } else if (
          aElement.classList.contains("av__calc--ashow") &&
          aElement.clientWidth + 2 < aElement.scrollWidth
        ) {
          tip =
            aElement.lastChild.textContent +
            " " +
            aElement.firstElementChild.textContent;
        } else if (aElement.getAttribute("data-type") === "setRelationCell") {
          const childElement = aElement.querySelector(".b3-menu__label");
          if (
            childElement &&
            childElement.clientWidth < childElement.scrollWidth
          ) {
        ...
        if (!tip) {
          ...
          if (href) {
            tip = `<span style="word-break: break-all">${href.substring(
              0,
              Constants.SIZE_TITLE
            )}</span>`;
          ...
          if (
            ...
            fetchPost("/api/asset/statAsset", { path: href }, (response) => {
              if (response.code === 1) {
                ...
                assetTip += ` ${response.data.hSize}${
                  title
                    ? '<div class="fn__hr"></div><span>' + title + "</span>"
                    : ""
                }<br>${window.siyuan.languages.modifiedAt} ${
                  response.data.hUpdated
                }<br>${window.siyuan.languages.createdAt} ${
                  response.data.hCreated
                }`;
              }
        ...
        notebookItemElement = hasClosestByClassName(
          event.target,
          "b3-list-item__text"
        );
        if (
          notebookItemElement &&
          notebookItemElement.parentElement.getAttribute("data-type") ===
            "navigation-root"
        ) {
          fetchPost(
            ...
            {
              notebook:
                notebookItemElement.parentElement.parentElement.getAttribute(
                  "data-url"
                ),
            },
            (response) => {
              const boxData = response.data.boxInfo;
              const tip = `${boxData.name} <small class='ft__on-surface'>${
                boxData.hSize
              }</small>${
                boxData.docCount !== 0
                  ? window.siyuan.languages.includeSubFile.replace(
                      "x",
                      boxData.docCount
                    )
                  : ""
              }<br>${window.siyuan.languages.modifiedAt} ${boxData.hMtime}<br>${
                window.siyuan.languages.createdAt
              } ${boxData.hCtime}`;
              const scopeNotebookItemElement = hasClosestByClassName(
                event.target,
                "b3-list-item__text"
              );
              if (
                notebookItemElement &&
                scopeNotebookItemElement &&
                notebookItemElement.isSameNode(scopeNotebookItemElement)
              ) {
                showTooltip(tip, notebookItemElement);
              }
              if (
                scopeNotebookItemElement &&
                scopeNotebookItemElement.parentElement.getAttribute(
                  "data-type"
                ) === "navigation-root" &&
                scopeNotebookItemElement.parentElement.parentElement.getAttribute(
                  "data-url"
                ) === boxData.id
              ) {
        ...
        const tipElement = hasClosestByAttribute(
          event.target,
          "id",
          "tooltip",
          true
...
const hidePopover = (event: MouseEvent & { path: HTMLElement[] }) => {
  ...
  const avPanelElement =
    hasClosestByClassName(target, "av__panel") ||
    hasClosestByClassName(target, "av__mask");
  if (avPanelElement) {
    // 浮窗上点击 av 操作，浮窗不能消失
    ...
    if (
      menuElement &&
      menuElement.getAttribute("data-name") !== "docTreeMore"
    ) {
      const blockPanel = window.siyuan.blockPanels.find((item) => {
  ...
  if (
    popoverTargetElement &&
    popoverTargetElement.classList.contains("b3-tooltips")
  ) {
    popoverTargetElement = undefined;
  }
  if (!popoverTargetElement) {
    popoverTargetElement = hasClosestByClassName(
      target,
      "popover__block"
    ) as HTMLElement;
  }
  ...
  if (
    !popoverTargetElement &&
    linkElement &&
    linkElement.getAttribute("data-href")?.startsWith("siyuan://blocks")
  ) {
  ...
  if (
    !popoverTargetElement ||
    (popoverTargetElement &&
      window.siyuan.menus.menu.data?.isSameNode(popoverTargetElement))
  ) {
    ...
    const blockElement = hasClosestByClassName(
      targetElement,
      "block__popover",
      true
    );
    ...
    if (blockElement) {
      for (let i = window.siyuan.blockPanels.length - 1; i >= 0; i--) {
        ...
        if (
          (item.targetElement || typeof item.x === "number") &&
          itemLevel >
            (maxEditLevels[item.element.getAttribute("data-oid")] || 0) &&
      ...
      for (let i = window.siyuan.blockPanels.length - 1; i >= 0; i--) {
        ...
        if (
          ...
          if (menuLevel && menuLevel >= itemLevel) {
            ...
            item.targetElement &&
            item.targetElement.classList.contains("protyle-wysiwyg__embed") &&
...
const getTarget = (
  ...
  aElement: false | HTMLElement
) => {
  if (
    window.siyuan.config.editor.floatWindowMode === 2 ||
  ...
  if (!popoverTargetElement && aElement) {
    if (
      ...
      const textElement = aElement.querySelector(
        ".av__celltext--url"
      ) as HTMLElement;
      if (
        textElement &&
  ...
  if (popoverTargetElement && getSelection().rangeCount > 0) {
    ...
    if (
      range.toString() !== "" &&
      popoverTargetElement.contains(range.startContainer)
    ) {
      return false;
    }
  }
...
export const showPopover = async (app: App, showRef = false) => {
  ...
  if (dataId) {
    ...
    popoverTargetElement
      ...
      ?.indexOf("virtual-block-ref") > -1
  ) {
    const nodeElement = hasClosestBlock(popoverTargetElement);
    if (nodeElement) {
      const postResponse = await fetchSyncPost(
    ...
    refDefs = [
      {
        refID: getIdFromSYProtocol(
          popoverTargetElement.getAttribute("data-href")
        ),
      },
    ];
  } else if (popoverTargetElement.dataset.type === "url") {
    // 在 database 的 url 列中以文枢协议开头的链接
    refDefs = [
      { refID: getIdFromSYProtocol(popoverTargetElement.textContent.trim()) },
    ];
  } else if (popoverTargetElement.dataset.popoverUrl) {
    // 镜像数据库
    const postResponse = await fetchSyncPost(
      popoverTargetElement.dataset.popoverUrl,
      { avID: popoverTargetElement.dataset.avId }
    );
    refDefs = postResponse.data.refDefs;
  } else {
    // pdf
    ...
    if (popoverTargetElement.classList.contains("protyle-attr--refcount")) {
      // 编辑器中的引用数
      targetId =
        popoverTargetElement.parentElement.parentElement.getAttribute(
          "data-node-id"
        );
      ...
      // 文件树中的引用数
      targetId =
        popoverTargetElement.parentElement.getAttribute("data-node-id");
    }
    if (url) {
