...
export const getAssetName = (assetPath: string) => {
  return pathPosix()
    .basename(assetPath, pathPosix().extname(assetPath))
    .replace(/-\d{14}-\w{7}/, "");
};
...
export const isLocalPath = (link: string) => {
  ...
  if (
    ...
    link.startsWith("\\\\") /* Windows 网络共享路径 */
  ) {
    return true;
  }
...
export const movePathTo = (
  ...
  const dialog = new Dialog({
    ...
    <input class="b3-text-field fn__block" style="padding-left: 42px;" value="" placeholder="${
...
<div class="b3-dialog__action">
  ...
  dialog.element
    .querySelector(".b3-dialog__header")
  ...
  if (paths && paths.length > 0) {
    fetchPost("/api/filetree/getHPathsByPaths", { paths }, (response) => {
      dialog.element.querySelector(
        ".b3-dialog__header .ft__smaller"
      ).innerHTML = escapeHtml(response.data.join(" "));
    });
  }
...
<li class="b3-list-item${
          html === "" ? " b3-list-item--focus" : ""
        }" data-path="/" data-box="${item.id}">
    <span class="b3-list-item__toggle b3-list-item__toggle--hl">
        <svg class="b3-list-item__arrow"><use xlink:href="#iconRight"></use></svg>
    </span>
    ${unicode2Emoji(
      item.icon || window.siyuan.storage[Constants.LOCAL_IMAGES].note,
...
<span class="counter counter--right b3-tooltips b3-tooltips__w" aria-label="${window.siyuan.languages.flashcardCard}">${item.flashcardCount}</span>`;
            ...
            fileHTML += `<li class="b3-list-item${
              fileHTML === "" ? " b3-list-item--focus" : ""
            }" data-path="${item.path}" data-box="${item.box}">
    ${unicode2Emoji(
      item.boxIcon || window.siyuan.storage[Constants.LOCAL_IMAGES].note,
      "b3-list-item__graphic",
      true
    )}
    <span class="b3-list-item__showall" style="padding: 4px 0">${escapeHtml(
      item.hPath
    )}</span>
    ${countHTML}
</li>`;
  ...
  const toggleMovePathHistory = () => {
    ...
    if (
      !keys ||
      keys.length === 0 ||
      (keys.length === 1 && keys[0] === inputElement.value)
    ) {
      return;
    }
    ...
    menu.addItem({
      ...
      click() {
        ...
        setStorageVal(
          Constants.LOCAL_MOVE_PATH,
          window.siyuan.storage[Constants.LOCAL_MOVE_PATH]
        );
      },
    });
    ...
    if (inputElement.value) {
      ...
      if (list.length > window.siyuan.config.search.limit) {
        list.splice(
          ...
          list.length - window.siyuan.config.search.limit
        );
      }
  ...
  inputElement.addEventListener("keydown", (event: KeyboardEvent) => {
    ...
    if (
      window.siyuan.menus.menu.element.getAttribute("data-name") ===
      "move-path-history"
    ) {
      return;
    }
    const currentPanelElement = searchListElement.classList.contains("fn__none")
      ? searchTreeElement
      : searchListElement;
    const currentItemElements = currentPanelElement.querySelectorAll(
      ".b3-list-item--focus"
    );
    ...
    if (searchListElement.classList.contains("fn__none")) {
      ...
      if (event.key === "ArrowLeft") {
        let parentElement =
          currentItemElement.parentElement.previousElementSibling;
        if (parentElement) {
          ...
          if (
            parentRect.top < fileRect.top ||
            parentRect.bottom > fileRect.bottom
          ) {
      ...
      if (event.key === "ArrowDown" || event.key === "ArrowRight") {
        ...
        if (nextElement.classList.contains("b3-list-item")) {
          ...
          if (
            nextRect.top < fileRect.top ||
            nextRect.bottom > fileRect.bottom
          ) {
      ...
      if (event.key === "ArrowUp") {
        ...
        while (previousElement) {
          if (previousElement.previousElementSibling) {
            if (
              previousElement.previousElementSibling.classList.contains(
                "fn__none"
              )
            ) {
              previousElement =
                previousElement.previousElementSibling as HTMLElement;
            } else {
              if (previousElement.previousElementSibling.tagName === "LI") {
                ...
                const liElements =
                  previousElement.previousElementSibling.querySelectorAll(
                    ".b3-list-item"
                  );
                previousElement = liElements[
                  liElements.length - 1
                ] as HTMLElement;
              }
              break;
        ...
        if (previousElement.classList.contains("b3-list-item")) {
          ...
          const fileRect = searchTreeElement.getBoundingClientRect();
          if (
            previousRect.top < fileRect.top ||
            previousRect.bottom > fileRect.bottom
          ) {
      ...
      if (event.key === "ArrowDown") {
        ...
        if (!currentItemElement.nextElementSibling) {
          ...
          currentItemElement.nextElementSibling.classList.add(
            "b3-list-item--focus"
          );
        }
        currentItemElement = currentPanelElement.querySelector(
          ".b3-list-item--focus"
        );
        if (
          currentPanelElement.scrollTop <
            currentItemElement.offsetTop -
              currentPanelElement.clientHeight +
              lineHeight ||
      ...
      if (event.key === "ArrowUp") {
        ...
        if (!currentItemElement.previousElementSibling) {
          ...
          currentPanelElement.children[length - 1].classList.add(
            "b3-list-item--focus"
          );
        } else {
          currentItemElement.previousElementSibling.classList.add(
            "b3-list-item--focus"
          );
        }
        currentItemElement = currentPanelElement.querySelector(
        ...
        if (
          currentPanelElement.scrollTop <
            currentItemElement.offsetTop -
              ...
              lineHeight ||
          currentPanelElement.scrollTop >
            currentItemElement.offsetTop - lineHeight * 2
        ) {
          currentPanelElement.scrollTop =
            currentItemElement.offsetTop - lineHeight * 2;
        }
  ...
  dialog.element.addEventListener("click", (event) => {
    ...
    while (target && !target.isEqualNode(dialog.element)) {
      if (target.classList.contains("b3-list-item__toggle")) {
        ...
        const currentPanelElement = searchListElement.classList.contains(
          "fn__none"
        )
          ? searchTreeElement
          : searchListElement;
        ...
        if (
          ...
          if (
            currentItemElements.length === 1 &&
            currentItemElements[0].isSameNode(target)
          ) {
            // 至少需选中一个
          } else {
...
const getLeaf = (liElement: HTMLElement, flashcard: boolean) => {
  ...
  if (toggleElement.classList.contains("b3-list-item__arrow--open")) {
    ...
    if (
      ...
      liElement.nextElementSibling.tagName === "UL"
    ) {
...
<span class="counter counter--right b3-tooltips b3-tooltips__w" aria-label="${window.siyuan.languages.flashcardCard}">${item.flashcardCount}</span>`;
        ...
        fileHTML += `<li data-box="${notebookId}" class="b3-list-item" data-path="${
          item.path
        }">
    <span style="padding-left: ${
      item.path.split("/").length * 8
    }px" class="b3-list-item__toggle b3-list-item__toggle--hl${
          item.subFileCount === 0 ? " fn__hidden" : ""
        }">
        <svg class="b3-list-item__arrow"><use xlink:href="#iconRight"></use></svg>
    </span>
    ${unicode2Emoji(
    ...
    <span class="b3-list-item__text ariaLabel" data-position="parentE" aria-label="${getDisplayName(
      ...
      true,
      true
    )} <small class='ft__on-surface'>${item.hSize}</small>${
          item.bookmark
            ? "<br>" + window.siyuan.languages.bookmark + " " + item.bookmark
            : ""
        }${
          item.name1
            ? "<br>" + window.siyuan.languages.name + " " + item.name1
            : ""
        }${
          item.alias
            ? "<br>" + window.siyuan.languages.alias + " " + item.alias
            : ""
        }${
          item.memo
            ? "<br>" + window.siyuan.languages.memo + " " + item.memo
            : ""
        }${
          item.subFileCount !== 0
            ...
            : ""
        }<br>${window.siyuan.languages.modifiedAt} ${item.hMtime}<br>${
          window.siyuan.languages.createdAt
        } ${item.hCtime}">${getDisplayName(item.name, true, true)}</span>
    ${countHTML}
</li>`;
      });
      if (fileHTML === "") {
      ...
      liElement.insertAdjacentHTML(
        "afterend",
        `<ul class="file-tree__sliderDown">${fileHTML}</ul>`
      );
      const nextElement = liElement.nextElementSibling;
      setTimeout(() => {
        nextElement.setAttribute(
          "style",
          `height:${nextElement.childElementCount * liElement.clientHeight}px;`
        );
        setTimeout(() => {
